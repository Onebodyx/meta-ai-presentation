...
### Этап 7. Минимальный UX-полиш для демонстрации
Цель: сделать интерфейс презентабельным, не усложняя реализацию.

Что сделать:

- выделить блоки: доска, история партии, кандидатные ходы, объяснение;
- показать, какой ход сделал пользователь и какой ответил движок;
- сделать формат объяснения легко читаемым;
- по возможности добавить короткий блок "что важно запомнить в этой позиции".

Критерий готовности:

- интерфейс читается с экрана без долгих пояснений;
- зритель быстро понимает, где ход, где анализ и где объяснение.

### Этап 8. Smoke-test и стабилизация
Цель: проверить, что демо переживет живой показ.

Что сделать:

- сыграть короткую пробную последовательность ходов;
- проверить обработку хотя бы одного нелегального ввода;
- проверить старт новой партии;
- проверить, что при повторном анализе интерфейс не рассинхронизируется;
- убедиться, что выбранный `hook` запускает легкую автоматическую проверку и не тормозит работу;
- убедиться, что при сбое `LLM API` приложение не падает полностью.

Критерий готовности:

- основной сценарий проходит без ручных исправлений в коде;
- есть понятный fallback на случай проблем с сетью или API;
- автоматическая проверка через `hook` пригодна для показа и не создает лишнего шума.
